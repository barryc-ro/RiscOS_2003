...
#if DEBUG
    fprintf(stderr, "antweb_handle_url for '%s'\n", href);
#endif
...
#if DEBUG
    fprintf(stderr, "doc click on item %p offset %d,%d error %p tag %d click fn %p\n",
...
static void antweb_unlink_doc(be_doc doc)
{
	...
	{
#if DEBUG
	    fprintf(stderr, "dispose_doc: dd %p dd->next %p\n", dd, dd->next);
#endif
	    if (dd->next == doc)
	    {
		dd->next = doc->next;
...
#if DEBUG
    fprintf(stderr, "dispose_doc: calling real dispose doc\n");
#endif
...
/*     IMGDBG(("animation: render background tile %d fill %d\n", do_tile, do_fill)); */
...
static void be_formater_loop(rid_header *rh, rid_text_item *ti)
{
        ...
        FMTDBG(("be_formater_loop(%p, %p) - sizing\n", rh, ti));
	...
	if (*fmt.width > rh->stream.fwidth)
	{
	    ...
	    FMTDBG(("be_formater_loop(%p, %p) - STOMPED sizing\n", rh, ti));
...
os_error *backend_goto_fragment(be_doc doc, char *frag)
{
	...
	if (first)
	{
	    ...
	    {
#if DEBUG
		fprintf(stderr, "Ensuring item visable, top at %d\n", bb.y1);
#endif
...
static void be_pparse_doc(antweb_doc *doc, int fh, int from, int to)
{
	...
	{
#if PP_DEBUG
	    fprintf(stderr, "Getting file data returned error: %s\n", ep->errmess);
	    fprintf(stderr, "In: file=%d, buffer=%p, number=%d, seq=%d\n",
		    fh, buffer, len, from);
	    fprintf(stderr, "Out: file=%d, buffer=%p, number=%d, seq=%d\n",
		    fh, buffer, len, from);
#endif
	}
	...
	fprintf(stderr, "setting margins to %d,%d\n", doc->margin.x0, doc->margin.y1);
#endif
    if (rh->margin.left != -1)
...
static void antweb_doc_progress(void *h, int status, int size, int so_far, int fh, int ftype, char *url)
{
	...
	if (doc->ph)
	{
		...
		{
#if DEBUG
		    fprintf(stderr, "Tail changed, checking for select items\n");
...
static access_complete_flags antweb_doc_complete(void *h, int status, char *cfile, char *url)
{
	...
	if (!need_to_parse)
	{
		...
		{
		    hiline = ((ooi == NULL ||
			       oti->line->top > osi->base.display->line->top) ?
			      oti->line :
			      osi->base.display->line );
		    frontend_view_bounds(doc->parent, &bb);
...
#if USE_MARGINS
	    if (doc->rh->margin.left != -1)
		doc->margin.x0 = doc->rh->margin.left*2;
	    if (doc->rh->margin.top != -1)
		doc->margin.y1 = - doc->rh->margin.top*2;
#if PP_DEBUG
	    fprintf(stderr, "setting margins to %d,%d\n", doc->margin.x0, doc->margin.y1);
#endif
#endif
...
#if 0				/* SJM: moved to top of function */
	ofs.action = 5;
	ofs.name = doc->cfile;
...
#if DEBUG
	fprintf(stderr, "Returned from frontend\n");
#endif
	backend_dispose_doc(doc);
    }
...
{
#if DEBUG
    fprintf(stderr, "Entering backend_doc_abort for %p\n", doc);
#endif
...
/*     new->url = strdup(url); */
...
#if DEBUG
    fprintf(stderr, "backend_open_url: checklist: add %p document_list %p next %p\n", new, document_list, document_list ? document_list->next : NULL);
#endif
	...
	*frag = 0;
...
#if DEBUG
    fprintf(stderr, "Calling access function flags %x\n", flags);
#endif
	...
	errnum = ep ? ep->errnum : -1;
	MemCheck_RestoreChecking(checking);
    }
    
    if (errnum == ANTWEB_ERROR_BASE + ERR_USED_HELPER)
...
#if LINK_DEBUG
    fprintf(stderr, "Highlight from item %p, flags=0x%x, line=%p\n", item, flags, item ? item->line : NULL);
#endif
...
#if LINK_DEBUG
    fprintf(stderr, "Start search at %p, aref=%p, line=%p\n", ti, aref, ti ? ti->line : NULL);
#endif
...
#if LINK_DEBUG
	fprintf(stderr, "ti=%p, next=%p, line=%p\n", ti, ti->next, ti->line);
#endif
    }
...
os_error *backend_activate_link(be_doc doc, be_item item, int flags)
{
	...
	return NULL;
#if DEBUG
    fprintf(stderr, "activate link %p (type %d)\n", item, item->tag);
#endif
