...
os_error *frontend_open_url(char *url, fe_view parent, char *target, char *bfile, char *bfiletype, int flags)
{
    os_error *ep;
...
/* 	} */
    ...
    ep = backend_open_url(parent, &parent->fetching, url, bfile, bfiletype, flags & fe_open_url_NO_REFERER ? NULL : referer, oflags);
...
os_error *fe_show_file(fe_view v, char *file, int no_history)
{
    ...
    ep = frontend_open_url(url, v, TARGET_TOP, NULL, NULL, fe_open_url_NO_CACHE | (no_history ? fe_open_url_NO_HISTORY : 0) | fe_open_url_NO_REFERER);
...
os_error *fe_show_file_in_frame(fe_view v, char *file, char *frame)
{
    ...
    ep = frontend_open_url(url, v, frame, NULL, NULL, fe_open_url_NO_CACHE | fe_open_url_NO_HISTORY | fe_open_url_NO_REFERER);
...
os_error *fe_internal_url_with_source(fe_view v, const char *internal, const char *target)
{
    ...
    return frontend_open_url(buffer, NULL, (char *)target, NULL, NULL, fe_open_url_NO_CACHE);
}
...
static os_error *fe__reload(fe_view v, void *handle)
{
	 ...
	 */
	    ...
	    */
	    ...
	    ep = frontend_complain(frontend_open_url(url, v, TARGET_SELF, NULL, NULL,
