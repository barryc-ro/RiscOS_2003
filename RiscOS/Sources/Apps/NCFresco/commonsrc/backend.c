...
os_error *antweb_handle_url(be_doc doc, rid_aref_item *aref, const char *query, const char *target)
{
	...
	{
	    e = frontend_open_url(dest, /* new_win?NULL:*/ doc->parent, (char *)target, NULL, NULL, 0);
	}
...
#if NEW_TEXTAREA
        otextarea_write_to_file_raw( tai, f );
#else
...
void antweb_submit_form(antweb_doc *doc, rid_form_item *form, int right)
{
    ...
    case rid_fm_GET:
		...
		ep = frontend_complain(frontend_open_url(dest2, doc->parent, target, NULL, NULL, fe_open_url_NO_CACHE));
    ...
    case rid_fm_POST:
		...
		*dest = 'y';
	    f = mmfopen(fname, "w");
            ...
            bMime = form->enctype && !strcasecomp( form->enctype, "multipart/form-data" );
	    ...
	    /* Never get a form query from the cache */
	    ep = frontend_complain(frontend_open_url(dest, doc->parent, target,
	                                             fname,
	                                             bMime ? (char*)MIME_TYPE : NULL,
	                                             fe_open_url_NO_CACHE));
	    mm_free(dest);
	}
	break;
    default:
...
static void be_refresh_document(int at, void *h)
{
	...
	frontend_complain(frontend_open_url(url, doc->parent, NULL, NULL, NULL, 1));
	mm_free(url);
    }
}
...
extern os_error *backend_open_url(fe_view v, be_doc *docp,
				  char *url, char *bfile, char *bfiletype,
				  char *referer,
				  int flags)
{
    antweb_doc *new;
