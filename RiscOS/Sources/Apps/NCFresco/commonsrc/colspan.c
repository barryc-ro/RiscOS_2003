...
extern int colspan_algorithm(rid_table_item *table, int slot, BOOL horiz)
{
	...
	for (x = 0; x < max; x++)
	{
		...
		/*const*/ int effective_start = group->istart == 0 ? 0 : the_cells[group->istart -1].leftmost;
		/*const*/ int my_leftmost = effective_start + group->width[slot];
...
static void colspan_init_structure1(rid_table_item *table, BOOL horiz)
{
    ...
    /* allocate and init storage */
    pcp_cell the_cells = (pcp_cell) mm_calloc(max, sizeof (struct pcp_cell_s));
