...
static void otextarea_write_to_file(Encoding *enc, rid_textarea_item *tai, FILE *f, BOOL raw)
{
    int i;
...
static void antweb_write_query(Encoding *enc, FILE *f, char *name, char *value, int *first, BOOL bMime)
{
    if (value == NULL)
	value = "";
...
static void antweb_write_textarea(Encoding *enc, FILE *f, rid_textarea_item *tai, int *first, BOOL bMime)
{
    if (tai->name == NULL)
	return;
    ...
    otextarea_write_to_file(enc, tai, f, !bMime);
}
    ...
    case rid_fm_POST:
		...
		*dest = 'y';
		...
		case rid_form_element_INPUT:
		{
		    ...
		    {
		    case rid_it_HIDDEN:
			antweb_write_query(enc, f, iis->name, iis->value, &first, bMime);
			break;
		    case rid_it_IMAGE:
			if (iis->data.image.x != -1)
			{
			    ...
			    strcat( buf3, (*buf3) ? ".x" : "x" );
			    sprintf(buf2, "%d", iis->data.image.x);
			    antweb_write_query(enc, f, buf3, buf2, &first, bMime);
		    ...
		    case rid_it_TEXT:
		    case rid_it_PASSWD:
			antweb_write_query(enc, f, iis->name, iis->data.str, &first, bMime);
			break;
		    case rid_it_CHECK:
		    case rid_it_RADIO:
			if (iis->data.radio.tick)
			{
			    antweb_write_query(enc, f, iis->name, iis->value ? iis->value : "on", &first, bMime);
			}
			break;
		    case rid_it_SUBMIT:
			if (iis->data.button.tick && iis->name)
		...
		case rid_form_element_SELECT:
		{
		    ...
		    for(ois = sis->options; ois; ois = ois->next)
			if (ois->flags & rid_if_SELECTED)
			    antweb_write_query(enc, f, sis->name, ois->value ? ois->value : ois->text, &first, bMime);
		    break;
		}
		...
		{
		    rid_textarea_item *tai = (rid_textarea_item *)fis;
		    antweb_write_textarea(enc, f, tai, &first, bMime);
		    break;
		}
		}
	    }
