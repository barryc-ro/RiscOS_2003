 ...
 *  CpmGetLptMask
 *
 *   Get a Mask byte of valid LPT ports from DOS/BIOS
 *
 *   NOTE: This should be moved to the client base Lpt support.
 *
 * ENTRY:
 *   pLptMask (input)
 *    Pointer to variable to place the LptMask byte
 *
 * EXIT:
 *   CLIENT_STATUS_SUCCESS - no error
 *
...
void
CpmGetLptMask( PUCHAR pLptMask )
{
#ifdef RISCOS
    *pLptMask = 1;	// off one piner only
#elif defined(DOS)
    UCHAR Mask = 0;
    unsigned int far *p;

   /*
    * Look at the BIOS data area for words at the following locations:
    *
    *   40:08 - LPT1 I/O Base address
    *   40:0A - LPT2 I/O Base address
    *   40:0C - LPT3 I/O Base address
    *   40:0E - LPT4 I/O Base address
    *
    * These are not vectors, but data to be read by peekw().
    * since we do not have peekw(), we will build the pointer ourselves.
    *
    * If the value at any of these locations is 0, then that LPT port
    * does not exist.
    *
    * If it is not zero, this is the base I/O address for the port and
    * it should exist. (as far as BIOS thinks so)
    *
    */

    p = (unsigned int far *)0x00400008L;
    if( *p != 0 ) {
        Mask |= 0x0001;  // LPT1 mask bit
    }

    p = (unsigned int far *)0x0040000AL;
    if( *p != 0 ) {
        Mask |= 0x0002;  // LPT2 mask bit
    }

    /*
     * The new client only offers LPT1, LPT2 to the host under
     * the old CPM protocol. This allows VDSPL to operate on the
     * old LPT4. The Old LPT3 is available for re-assignment.
     */
...
#else
    /*
     * Say all LPT's are available for Windows. (PrintMan sets these up)
     * This way a user can create one with printman and not have to exit the
     * client for the port share to be available.
     *
     * If we only return currently valid ports, the host will not allow
     * the new ports to be attached to by the user dynamically.
     */
    *pLptMask = 0x0003;
    return;
#endif
 ...
 *  CpmGetComMask
 *
 *   Get a Mask byte of valid COM ports from DOS/BIOS
 *
 *   NOTE: This should be moved to the client base Lpt support.
 *
 * ENTRY:
 *   pComMask (input)
 *    Pointer to variable to place the ComMask byte
 *
 * EXIT:
 *   CLIENT_STATUS_SUCCESS - no error
 *
...
CpmGetComMask( PUCHAR pComMask )
{
    ...
    * Look at the BIOS data area for words at the following locations:
    *
    *   40:00 - COM1 I/O Base address
    *   40:02 - COM2 I/O Base address
    *   40:04 - COM3 I/O Base address
    *   40:06 - COM4 I/O Base address
    *
    * These are not vectors, but data to be read by peekw().
    * since we do not have peekw(), we will build the pointer ourselves.
    *
    * If the value at any of these locations is 0, then that COM port
    * does not exist.
    *
    * If it is not zero, this is the base I/O address for the port and
    * it should exist. (as far as BIOS thinks so)
    ...
    */
    ...
    p = (unsigned int far *)0x00400000L;
    if( *p != 0 ) {
        Mask |= 0x0001;  // COM1 mask bit
    }

    p = (unsigned int far *)0x00400002L;
    if( *p != 0 ) {
        Mask |= 0x0002;  // COM2 mask bit
    }

    /*
     * The new client only offers COM1, COM2 to the host under
     * the old CPM protocol. This allows VDCCM to operate on the
     * old COM4. The Old COM3 is available for re-assignment.
     */
...
#else
    /*
     * Say all COM's are available for Windows. (PrintMan sets these up)
     * This way a user can create one with printman and not have to exit the
     * client for the port share to be available.
     *
     * If we only return currently valid ports, the host will not allow
