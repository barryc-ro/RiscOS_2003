...
STATICF void sysspChildHandler()
{
   ...
   */
  if (waitid(P_ALL, (id_t) 0, &stat, WEXITED) == 0)
    {
      psts = (sysspProcSts *) ysHshFind(sysspcx, &stat.si_pid, sizeof(pid_t));
      if (psts)
	psts->sts = (stat.si_code == CLD_EXITED
		     ? SYSSP_STS_EXITOK : SYSSP_STS_EXITNOTOK);
    }
}
