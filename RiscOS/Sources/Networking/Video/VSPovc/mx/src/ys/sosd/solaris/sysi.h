...
#define ORASTDIO
#endif
#ifndef ORASYS_SYSTEMINFO
#include <sys/systeminfo.h>
#define ORASYS_SYSTEMINFO
#endif
#ifndef ORAUNISTD
#include <unistd.h>
#define ORAUNISTD
...
/*
 * The following definitions are missing from the vendor's headers.
 */
...
#   undef offsetof
# endif
# define offsetof(type, field)  ((size_t) 0)
#endif
...
#define syspGetHostName(o, b, l)  (sysinfo(SI_HOSTNAME, (b), (l)) > 0)
#define syspGetPid(o, b, l)       (sprintf((b), "%d", getpid()), TRUE)
#define syspGetAffinity(o, b, l)  (FALSE)
boolean syspGetCpuTime(dvoid *osdp, sysb8 *cputm);
boolean syspGetMemUsage(dvoid *osdp, ub4 *kbytes);
...
#define syslPrint(fmt, ap)  ((void) vfprintf(stdout, fmt, ap))
#define syslError(fmt, ap)  ((void) vfprintf(stderr, fmt, ap))
void    syslConsole(sword level, const char *fmt, va_list ap);

boolean syslGetc(dvoid *osdp, sword *ch, ub4 delay);
#define syslDetach()        ((void) (setsid()))
 ...
 */
#define systmGetTicks()  ((ub4) (gethrtime() / 1000))
boolean systmGetClock(sysb8 *clock);
void    systmClockToTime(sysb8 *clock, ystm *sttm);
 ...
 */
typedef sigset_t sysimsk;
extern  sysimsk sysiNoMask;
sysimsk sysiDisable(dvoid *osdp);
void    sysiEnable(dvoid *osdp, sysimsk mask);
void    sysiPause(dvoid *osdp, sysimsk mask, ub4 delay);
