#ifdef RCSID
static char *RCSid = 
"$Header: sysfp.c@@\main\oms_mx3.3_dev\oms_mx3.3_nt_dev\0 \
Checked out on Tue Jan 27 1997 11:2 by jchangav \
 ...
 *
 * MODIFIED
 * dbrower   09/ 9/96 - remove unused stuff, update to 3.2 interfaces.
 * syen  11/14/95 - port to WinNT.
 */
...
#define S_ISDIR(mode)   ((mode&0xF000) == 0x4000)    /* from sys/stat.h */
 ...
 */
boolean sysfpSeek(sysfp *fp, sysb8 *offset)
{
...
yslst *sysfpGetDir(CONST char *fn)
{
  ...
  /* need to append \\* to search the directory */
  strcpy(pathname, fn);
  strcat(pathname, "\\*");
  hSearch = FindFirstFile(pathname, &fd);
  ...
  yseTry
    {
      ...
      ysLstEnq(lst, (dvoid *) ysStrDup(fd.cFileName));

      while (FindNextFile(hSearch, &fd))
        ysLstEnq(lst, (dvoid *) ysStrDup(fd.cFileName));

      DISCARD FindClose(hSearch);
    }
	       ...
	       CONST char *sfx)
{
  ...
  /* prepend the path */
  if (*base == delim || (base[1] == ':' && base[2] == delim) || !path)
    DISCARD strcpy(result, base);
  else if (base[1] == ':' && base[2] != delim)
    {
      /* relative path on a different drive: get the absolute path */
      char absPath[_MAX_PATH], *pabsPath;
      
      pabsPath = _getdcwd( toupper(base[0]) - 'A' + 1, absPath, _MAX_PATH);
      if (!pabsPath)
        result[0] = '\0';
      else
        DISCARD strcpy(result, absPath);
    }
  else
    {
      if (path[1] == ':' && path[2] != delim)
      {
        /* relative path on a different drive: get the absolute path */
        char absPath[_MAX_PATH], *pabsPath;
      
        pabsPath = _getdcwd( toupper(path[0]) - 'A' + 1, absPath, _MAX_PATH);
        if (!pabsPath)
          result[0] = '\0';
        else
        {
...
void sysfpExtractPath(char *path, CONST char *fn)
{
  ...
  for (s = fn + strlen(fn); s > fn && *s != delim; s--) ;
  if (s == fn)
    DISCARD strcpy(path, ".");
  else
    {
...
void sysfpExtractFile(char *file, CONST char *fn)
{
  ...
  for (s = fn + strlen(fn); s > fn && *s != delim; s--) ;
  DISCARD strcpy(file, (*s == delim ? s + 1 : s));
}
